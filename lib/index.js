...
import coreFonts from '@nrk/core-fonts/lib/core-fonts.min.css'
...
body.className = 'nrk-color-base' // Ensure Firefox darkmode does not get black background
body.innerHTML = `
...
function chooseTransform (callback) {
  try {
    ...
    script.onload = () => callback(window.Babel.transform)
...
function exec (scripts, transform, callback) {
  setTimeout(() => {
    ...
    if (!source) return callback && callback()
      ...
      const transpiledCode = transform(source.textContent, { presets: ['es2015', 'react'] }).code
      target.appendChild(document.createTextNode(transpiledCode))
    }
