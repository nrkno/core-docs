...
mark.paragraph = (text) => `<p class="docs-paragraph">${text}</p>`
...
function generateTabs (html) {
  ...
  queryAll(dom.children).forEach((el) => {
    ...
    if (el.className.indexOf('docs-heading--4') !== -1) {
      ...
      tabs.insertAdjacentHTML('beforeend', `<a id="${a.id}" aria-controls="${a.id}--panel" href="#${a.id}">${a.textContent}</a>`)
      tabs.insertAdjacentHTML('afterend', `<div id="${a.id}--panel" aria-labelledby="${a.id}"></div>`)
      dom.removeChild(el)
    } else if (tabs) {
...
function renderPage (event) {
  const html = event.target.responseText.replace(/<!--\s*demo\n|\ndemo\s*-->/g, '')
